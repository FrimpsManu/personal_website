  ...
  return (
    <>
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ease-in-out ${
        ...
        scrolled 
          ? 'bg-white/80 dark:bg-neutral-900/80 backdrop-blur-xl border-b border-neutral-200/50 dark:border-neutral-800/50 shadow-lg shadow-neutral-900/5' 
          : 'bg-transparent'
      }`}>
            ...
            <div className="hidden lg:flex items-center space-x-1">
              {navItems.map((item, index) => (
                <button
                  ...
                  className={`relative px-4 py-2 text-sm font-medium rounded-lg transition-all duration-300 group hover:scale-105 ${
            ...
            <div className="px-4 py-3 space-y-1">
              {navItems.map((item, index) => (
                <button
                  key={item.id}
                  onClick={() => scrollToSection(item.id)}
                  className={`w-full flex items-center gap-3 px-4 py-3 text-left rounded-lg transition-all duration-300 hover:scale-[1.02] ${
                    activeSection === item.id
                      ...
                      : 'text-neutral-700 dark:text-neutral-300 hover:text-primary-600 dark:hover:text-primary-400 hover:bg-gradient-to-r hover:from-neutral-50 hover:to-neutral-100 dark:hover:from-neutral-800/50 dark:hover:to-neutral-700/50'
                  }`}
                  style={{ 
      ...
      <div className="hidden xl:flex fixed right-8 top-1/2 transform -translate-y-1/2 flex-col gap-3 z-40">
        {navItems.map((item, index) => (
          <button
            key={item.id}
            onClick={() => scrollToSection(item.id)}
            className={`group relative w-3 h-3 rounded-full transition-all duration-300 hover:scale-150 ${
              activeSection === item.id
